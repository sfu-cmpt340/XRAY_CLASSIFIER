...
# -- C) PyTorch model (dense_net.pth)
MODEL_PATH = "dense_net.pth"  # Adjust path if needed

class XRayDenseNet(nn.Module):
    def __init__(self, num_classes=4):
        super(XRayDenseNet, self).__init__()
...
try:
    dense_net_model = XRayDenseNet(num_classes=4)
    dense_net_model.load_state_dict(torch.load(MODEL_PATH, map_location=DEVICE))
    dense_net_model.to(DEVICE)
    dense_net_model.eval()
except Exception as e:
    dense_net_model = None
    print("ERROR loading dense_net.pth. Make sure you have the correct file or model definition.")
    print(e)
...
# For backward compatibility with our new evaluation function name.
evaluate_model_on_test_folder = evaluate_model_on_test_folder = evaluate_model_on_test_folder = evaluate_model_on_test_folder = evaluate_model_on_test_folder = evaluate_model_on_test_folder = evaluate_model_on_test_folder = evaluate_model_on_test_folder
